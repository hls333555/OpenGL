...
namespace test
{
		...
		, m_ModelRotSpeed(90.f)
	{
		...
		m_CubeShader.reset(new Shader("res/shaders/LightingWithShadows.shader"));
		m_CubeShader->Bind();
		...
		glTextureParameteri(m_DepthTextureBuffer, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTextureParameteri(m_DepthTextureBuffer, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
